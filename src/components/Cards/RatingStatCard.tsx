      ...
      <div className="text-lg font-medium rounded-2xl w-[330px] max-w-full p-[10px]">
        {ratingStatRev?.map((val, idx) => (
          <div key={idx} className="flex justify-between items-center py-[5px] px-[8px] gap-[10px]">
            <div className="text-black-300 text-lg font-medium text-left w-[30px] shrink-0 pc:text-xl">
              {val.rating}점
            </div>
            ...
            <div className="text-gray-300 text-lg font-bold text-left w-[30px] shrink-0 pc:text-xl">
              {val.count}
            </div>
          </div>
        ))}
